...
import { pigVaccineService } from '../../services/pigVaccineService';
...
const PigVaccines = () => {
  ...
  const fetchVaccines = async () => {
    try {
      ...
      const response = await pigVaccineService.getAll(params);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingVaccine) {
        await pigVaccineService.update(editingVaccine.id, formData);
        alert('Vaccine updated successfully');
      } else {
        await pigVaccineService.create(formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await pigVaccineService.delete(id);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Pig Vaccines</h1>
        <button
          onClick={() => {
            resetForm();
            setShowModal(true);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-4">
              {editingVaccine ? 'Edit Pig Vaccine' : 'Create New Pig Vaccine'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Classical Swine Fever (CSF)"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500"
                  placeholder="e.g., CSF Vaccine (Live Attenuated)"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500"
                  placeholder="e.g., ဝက်ကျောက်ရောဂါ ကာကွယ်ဆေး"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500"
                  placeholder="e.g., Piglets, Breeding Stock"
                />
                <p className="mt-1 text-xs text-gray-500">
                  Enter target stages separated by commas (e.g., Piglets, Sows, Growers)
                </p>
              </div>
